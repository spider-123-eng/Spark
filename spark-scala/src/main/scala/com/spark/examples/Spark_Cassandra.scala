...
object Spark_Cassandra {
  ...
  def main(args: Array[String]) {
    ...
    try {
      ...
      val upate = sc.parallelize(Seq(Employee(328561, "Revanth Reddy", 12345), Employee(328565, "Hari", 9999999)))
      ...
      //Column selection
      sc.cassandraTable[Employee](KEY_SPACE_NAME, TABLE_NAME).select("id", "name", "salary").take(10).foreach(println)
